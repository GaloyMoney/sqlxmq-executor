...
impl<'a> JobExecutor<'a> {
    ...
    async fn checkpoint_attempt<T: Serialize, E: JobExecutionError>(
        ...
        if let Some(tracing_data) = data.job_meta.tracing_data.as_ref() {
            crate::otel::inject_tracing_data(&span, tracing_data);
        } else {
            crate::otel::inject_tracing_data(&span, &data.tracing_data);
        }
        ...
        data.job_meta.tracing_data = Some(crate::otel::extract_tracing_data());
